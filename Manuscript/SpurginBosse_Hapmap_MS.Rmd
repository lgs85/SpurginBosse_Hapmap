...
Fig1A <- get_map(location = 'Europe', zoom = 4,maptype="roadmap", color="bw") %>%
ggmap()+ 
geom_point(aes(x = V3,y = V2),data = latlong,size = 2,col = "red")+
  xlab("Longitude")+
  ylab("Latitude")
...
Fig1B <- subset(ibd,pop1 == "Turkey" | pop2 == "Turkey") %>%
          ggplot(aes(x = dkm,y = fst,col = island))+
            geom_point()+
            theme_bw()+
            theme(legend.position = "none")+
            ylab(expression(F[ST]))+
