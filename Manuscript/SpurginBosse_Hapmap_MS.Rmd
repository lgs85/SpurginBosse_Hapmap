...
```{r Figure 1,fig.width = 8, fig.height = 8}
Fig1A <- get_map(location = 'Europe', zoom = 4) %>%
ggmap()+
geom_point(aes(x = Lat,y = Long),data = ll,size = 2,col = "red")+
  xlab("Longitude")+
  ylab("Latitude")

Fig1B <- subset(hetlong,pop != "Bulgaria") %>%
  ggplot(aes(x = pop,y = he))+
...
```{r Figure 2,fig.width = 7,fig.height = 6}
...
**Figure XX** Population structure in European Great tit populations. **A** Pairwise \(F_{ST}\) between all European populations in relation to pairwise geographic distance; **B** pairwise \(F_{ST}\) between a proposed refugial population in Turkey and all other populations, in relation to geographic distance from the refugia. Blue points in **A** and **B** are comparisons involving the island populations of Corsica and Crete (NOTE - SARDINIA NOT SEPARATED FROM ITALY IN THESE PLOTS YET), while grey points represent all other comparisons. **C** Output from admixture analysis at *K* = 6.
